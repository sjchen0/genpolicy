...
def _run(local_rank, gloab_rank, world_size, cfg):
    ...
    policy_num_parameters = sum(p.numel() for p in policy_model.parameters())
    mprint(f"Number of parameters in RADD: {radd_num_parameters}, in PolicyNet: {policy_num_parameters}")

    ema = ExponentialMovingAverage(
        policy_model.parameters(), decay=cfg.training.ema)
    mprint(radd_model)
    mprint(policy_model)
    mprint(f"EMA: {ema}")
    ...
    optimizer = losses.get_optimizer(cfg, policy_model.parameters())
    mprint(f"Optimizer: {optimizer}")
    scaler = torch.cuda.amp.GradScaler()
    mprint(f"Scaler: {scaler}")
    state = dict(optimizer=optimizer, scaler=scaler, score_model=radd_model, policy_model=policy_model, noise=noise, ema=ema, step=0) 
    ...
    while state['step'] < num_train_steps + 1:
        ...
        if step != state['step']:
            ...
            if step > 0 and step % cfg.training.snapshot_freq == 0 or step == num_train_steps:
                # Save the checkpoint.
                if gloab_rank == 0:
                    utils.save_policy_checkpoint(os.path.join(
